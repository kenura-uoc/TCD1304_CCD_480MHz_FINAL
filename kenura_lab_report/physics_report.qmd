...
### Phase 2 — Combined Dual-Analyte Model {#sec-phase2-model}
Since every spinach-extract solution contains both Chl-a and Chl-b, and the UV-Vis calibration independently provides both concentrations per sample, the dataset already constitutes mixed-pigment training data. The Phase 2 architecture uses a **single dual-output model** with a two-channel input (one spectrum per laser) that predicts $[\text{Chl-a}]$ and $[\text{Chl-b}]$ simultaneously in mg/L.

#### Physical Effects in a Mixed Solution
Unlike single-analyte measurements, a combined Chl-a + Chl-b solution introduces several coupled spectral phenomena that must all be accounted for in the model design:

**1. Spectral Superposition.** Both analytes emit fluorescence simultaneously. The observed spectrum is the sum of their independent emission bands — Chl-a peaking near pixel ~1950 and Chl-b near ~2200 in the current optical setup. The total signal therefore contains information about *both* concentrations in every pixel range.

**2. Cross-Excitation.** Neither laser is perfectly selective:
...
**4. Total-Concentration Quenching.** At high combined pigment densities, molecular aggregation and collisional energy transfer pathways suppress overall fluorescence yield. This quenching is proportional to the *total* chlorophyll load $([\text{Chl-a}] + [\text{Chl-b}])$, further coupling the two concentration-dependent signals.

**5. Independent Red Shifts.** Each analyte's peak shifts to longer wavelengths with increasing *own* concentration (self-absorption / IFE). Chl-a shifts at ~12 px/(mg/L) and Chl-b at ~40 px/(mg/L). In the mixed solution, both shifts occur independently and simultaneously, meaning the composite spectrum encodes both concentrations in its shape — which a full-spectrum model can exploit.

#### Ideal Simulation Benchmark {#sec-phase2-sim}
The ideal combined-solution scenario was simulated in `ml_model/simulation_combined.py` with 300 synthetic mixed-solution spectra at 128× frame averaging. Two dual-output models were compared on the two-channel concatenated feature vector (4400 features):
...
#### Cross-Excitation: Is It Fixed or Variable?
Cross-excitation efficiency ($\varepsilon_\text{cross}$) is **fundamentally a fixed physical property** — it is determined entirely by the molar absorption spectrum of each pigment at each laser wavelength. For the current setup: $\varepsilon(\text{Chl-b at 405 nm}) \approx 0.15$ and $\varepsilon(\text{Chl-a at 450 nm}) \approx 0.25$, derived from published absorption coefficients [13, 8].

However, in practice $\varepsilon_\text{cross}$ *appears variable* for the following reasons:

*   **Solvent effects**: Switching from acetone (current) to DMSO shifts the Soret absorption peak by approximately 5–8 nm [13], changing the overlap integral with each laser line and altering $\varepsilon_\text{cross}$ by up to ±5%.
*   **Pigment degradation**: Chlorophyll degrades to pheophytin (loss of Mg²⁺) whose Soret band red-shifts by ~15 nm. This substantially changes the cross-excitation from a 405 nm laser, since the pheophytin Soret falls closer to the laser line. Solutions older than 4–6 hours show measurable drift in $\varepsilon$.
*   **Aggregation at high concentrations**: Above ~10 mg/L, chlorophyll-chlorophyll aggregation broadens and red-shifts the Soret band, altering the excitation efficiency in a concentration-dependent manner [7, 11].

**How to measure $\varepsilon_\text{cross}$ experimentally**: Prepare a **pure single-analyte solution** (e.g., Chl-a only, verified by UV-Vis) and illuminate it under *both* lasers sequentially. The ratio of peak signal under the "wrong" laser (450 nm) to the "correct" laser (405 nm) gives $\varepsilon_\text{cross}$ directly. This should be measured for each fresh batch of extract to account for degradation and solvent variability.
