...
PLOTS_DIR       = SCRIPT_DIR / "training_plots_v4"
...
def clean_spectra(spectra, concs, name=""):
    ...
    s1_mask     = np.abs(peak_pixels - med) <= 100.0 * (mad + 1e-8)
...
def full_evaluation(model, X, y, name):
    ...
    # ── PLOT 1: Predicted vs Actual + Residuals (3 panels) ───────────────────
    ...
    # Panel 3: Residuals in TRANSFORMED space — should look flat/random if transform worked
    ax3 = axes[2]
    ...
    ax3.axhline( rmse_t,  color='green',  linestyle=':',  lw=1,
                label=f'+RMSE_t ({rmse_t:.3f})')
    ax3.axhline(-rmse_t,  color='green',  linestyle=':',  lw=1,
                label=f'-RMSE_t ({rmse_t:.3f})')
    ax3.set_xlabel("Actual Concentration (ppm)")
    ax3.set_ylabel(f"Residual in {Y_TRANSFORM}-space")
    ax3.set_title(f"Residuals in {Y_TRANSFORM}-transformed space\n"
                  f"(should be flat/random — no funnel → transform worked)")
    ...
    # ── PLOT 1: Predicted vs Actual + Residuals ──────────────────────────────
    ...
    fig2.suptitle(f"{name} — CV Concentration Outputs (sorted by actual concentration)",
                  fontsize=13, fontweight='bold')

    cmap_bar = plt.colormaps['viridis']
    bar_colors = [cmap_bar(v / y_sorted.max()) for v in y_sorted]

    # Panel 1: Actual vs Predicted bars
    ax3 = axes2[0]
    x = np.arange(n_samples)
    width = 0.38
    ax3.bar(x - width/2, y_sorted,    width, label='Actual',    color='steelblue',  alpha=0.85, edgecolor='k', lw=0.3)
    ax3.bar(x + width/2, pred_sorted, width, label='CV Predicted', color='darkorange', alpha=0.85, edgecolor='k', lw=0.3)
    ax3.set_ylabel("Concentration (ppm)")
    ax3.set_title("Actual vs CV Predicted Concentration")
    ax3.legend(); ax3.grid(True, alpha=0.25, axis='y')
    ax3.spines['top'].set_visible(False); ax3.spines['right'].set_visible(False)

    # Panel 2: Error per sample
    ax4 = axes2[1]
    bar_cols = ['tomato' if e > 0 else 'steelblue' for e in err_sorted]
    ax4.bar(x, err_sorted, color=bar_cols, edgecolor='k', lw=0.3, alpha=0.85)
    ax4.axhline(0,          color='black',  lw=1.0)
    ax4.axhline( cv_rmse,   color='red',    lw=1.2, linestyle='--', label=f'+RMSE ({cv_rmse:.3f})')
    ax4.axhline(-cv_rmse,   color='red',    lw=1.2, linestyle='--', label=f'-RMSE ({cv_rmse:.3f})')
    ax4.set_ylabel("Error (ppm)  [pred − actual]")
    ax4.set_title("Prediction Error per Sample  (red=overestimate, blue=underestimate)")
    ax4.legend(fontsize=9); ax4.grid(True, alpha=0.25, axis='y')
    ax4.spines['top'].set_visible(False); ax4.spines['right'].set_visible(False)

    # Panel 3: % error
    ax5 = axes2[2]
    ax5.bar(x, pct_err, color=bar_colors, edgecolor='k', lw=0.3, alpha=0.9)
    ax5.axhline(10, color='red',    lw=1.2, linestyle='--', label='10% threshold')
    ax5.axhline(5,  color='orange', lw=1.0, linestyle=':',  label='5% threshold')
    ax5.set_xlabel("Sample Index (sorted by actual concentration)")
    ax5.set_ylabel("Absolute % Error")
    ax5.set_title("Absolute % Error per Sample")
    ax5.set_xticks(x)
    ax5.set_xticklabels([f"{v:.2f}" for v in y_sorted], rotation=70, fontsize=7)
    ax5.set_xlabel("Actual Concentration (ppm)")
    ax5.legend(fontsize=9); ax5.grid(True, alpha=0.25, axis='y')
    ax5.spines['top'].set_visible(False); ax5.spines['right'].set_visible(False)
    ...
    # ── Save CSV of all outputs ───────────────────────────────────────────────
    df_out = pd.DataFrame({
        "sample_rank"       : sample_ids,
        "actual_ppm"        : y_sorted,
        "cv_predicted_ppm"  : np.round(pred_sorted, 4),
        "error_ppm"         : np.round(err_sorted,  4),
        "abs_error_ppm"     : np.round(abs_err,      4),
...
def train_and_save():
    ...
    if len(Xa) == 0:
        ...
        # Train in transformed space — save both model AND transform name
        ...
        joblib.dump({
            'model'    : final_pls_a,
            'transform': Y_TRANSFORM,   # 'log', 'sqrt', or 'none'
        }, OUTPUT_MODEL_A)
        print(f"  Saved Chl-A model → {OUTPUT_MODEL_A}  (transform='{Y_TRANSFORM}')")
    ...
    if len(Xb) == 0:
        ...
        # Save final model with transform metadata
        ...
        print(f"  Saved Chl-B model → {OUTPUT_MODEL_B}  (transform='{Y_TRANSFORM}')")
